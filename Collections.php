...
<body>
    ...
    <section class="my-10">
        ...
        <div class="flex flex-col md:flex-row gap-5 justify-between max-w-[1300px] mx-auto mb-10 px-4">
            <h1 class="text-3xl">Sunshine Serves</h1>
        ...
        if ($color_option != '') {
            $product_select .= " WHERE Color = '$color_option'";
        }
        ...
        switch ($sort_option) {
            ...
            case 'highest':
                ...
                break;
            case 'discount':
                $sort_clause .= " WHERE DiscountPrice > 0";
                break;
            case 'extended_sizing':
                $product_select .= " WHERE ExtendedSizing = 'true'";
                break;
            case 'selling_fast':
                $product_select .= " WHERE SellingFast = 'true'";
                break;
            case 'more_colors':
                $product_select .= " WHERE MoreColors = 'true'";
                break;
            default:
            ...
            <?php if ($count > 0) { ?>
                ...
                <?php if ($count > $display_limit) { ?>
                    <div class="flex justify-center mt-5 select-none">
                        <button id="see-more-btn" class="border-2 border-indigo-500 text:md sm:text-xl font-semibold px-20 sm:px-24 py-2">See More</button>
                    </div>
                <?php } ?>
            <?php } else { ?>
