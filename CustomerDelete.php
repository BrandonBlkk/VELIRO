...
if (isset($_POST['confirmDelete'])) {
    ...
    if ($deleteCartQuery && $deleteFavoritesQuery && $deleteReviewsQuery && $deleteOrderDetailsQuery && $deleteOrdersQuery) {
        ...
        if ($deleteCustomerQuery) {
            ...
            session_destroy();
            $_SESSION['alert_message'] = "Your account has been successfully deleted.";
            $_SESSION['alert_class'] = "bg-green-200 border-green-400 text-green-800";
            echo "<script>window.location = 'Home.php'</script>";
        } else {
            echo "<script>window.alert('Failed to delete account. Please try again.')</script>";
            echo "<script>window.location = 'CusProfile.php'</script>";
        }
    } else {
    ...
    <div class="bg-white p-6 rounded shadow-md w-full max-w-md">
        ...
        <form action="" method="post">
            ...
            <div class="flex justify-end space-x-4">
                <a href="CusProfile.php" class="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300 transition duration-200">Cancel</a>
